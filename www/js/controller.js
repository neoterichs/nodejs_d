...
.controller('PageCtrl', function($scope,$ionicFrostedDelegate,$ionicScrollDelegate,$rootScope,$http) {
	$rootScope.$on('newchatm', function (event, args) {
		console.log("dddd")
		$scope.messages.push(angular.extend({content:args.message}));
	...
	$scope.rmessages = receiveOptions.slice(0, receiveOptions.length);

 	var id = 0;
	$scope.dochat = function(user){
		console.log(user.chatm);
		if(user.chatm != ""){
			...
			var data_parameters = "slocid="+slocid+ "&orgid="+orgid+ "&fromid="+userid+ "&toid="+toid+ "&message="+user.chatm;
			console.log(data_parameters);
			$http.post("http://"+globalip+"/send_chat_message",data_parameters, {
				headers: {'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'}
			})
			.success(function(response) {
				if(response[0].status == "Y"){
					$scope.user = {chatm: ''};
					$scope.messages.push(angular.extend({content:user.chatm}));
